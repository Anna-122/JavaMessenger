...
public class Server {
     ...
     */
    private static final org.apache.logging.log4j.Logger LOG4j2 = LogManager.getLogger(Server.class);
    ...
    protected void startServer(int port) {
        try {
            ...
            LOG4j2.info("Server started.\n");
        } catch (Exception e) {
            gui.refreshDialogWindowServer("Failed to start server.\n");
            LOG4j2.error("Failed to start server.\n");
        }
    }
    ...
    protected void stopServer() {
        try {
            ...
            if (serverSocket != null && !serverSocket.isClosed()) {
                ...
                LOG4j2.info("Server stopped.");
            } else gui.refreshDialogWindowServer("The server is not running - there is nothing to stop!\n");
            LOG4j2.error("The server is not running - there is nothing to stop!\n");
        } catch (Exception e) {
    ...
    protected void acceptServer() {
        while (true) {
            try {
                ...
                LOG4j2.error("Server connection lost.\n");
                break;
            }
        }
    }
    ...
    protected void sendMessageAllUsers(Message message) {
        for (Map.Entry<String, Connection> user : model.getAllUsersMultiChat().entrySet()) {
            try {
                ...
                LOG4j2.error("Error sending message to all users!\n");
            }
        }
    }
    ...
    private class ServerThread extends Thread {
        ...
        private String requestAndAddingUser(Connection connection) {
            while (true) {
                try {
                    ...
                    LOG4j2.error("An error occurred while requesting and adding a new user\n");
                }
            }
        }
            ...
            while (true) {
                try {
                    ...
                    if (message.getTypeMessage() == MessageType.DISABLE_USER) {
                        ...
                        LOG4j2.info("Remote access user %s disconnected.\n");
                        break;
                    }
                } catch (Exception e) {
                    ...
                    LOG4j2.error("An error occurred while sending a message from the user either disconnected!\n");
                    break;
                }
            }
        }
        ...
        public void run() {
            ...
            LOG4j2.info("A new user connected with a remote socket\n");
            try {
                ...
                LOG4j2.error("An error occurred while sending a message from the user!\n");
            }
        }
    }
}
